...
public class FlightMethods extends Connect {
    ...
    public void insertFlight(Flights flights) {
        ...
        try (PreparedStatement preparedStatement = connection().prepareStatement(sqlInsertFlight)) {
            ...
            System.out.println("Введите код аэропорта откуда будет вылет самолёта например{NQZ}:");
            preparedStatement.setString(3, flights.setAirportDepartureCode(sc.nextLine()));

            System.out.println("Введите код аэропорта куда будет происходить прилёт самолёта например{TAS}:");
            ...
            System.out.println("Введите номер рейса (должен быть уникальным) например{FV 9263}");
